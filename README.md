...
### ✨ 云端核心特性
1. **自动热迁移 (Auto-Migration)**：如果在已经有很多本地 `uploads/` 图包的老服务器上首次配置了 S3 参数，重启 Node 后，系统会在后台默默将所有本地图包自动上传到远端桶，并重写数据库，完成后自动摧毁本地冗余原件。完全是零交互平滑升级！
2. **智能云垃圾回收 (Cloud Garbage Collection)**：当你在前端时间线删除了一条带有九宫格照片的动态，或者在编辑状态删掉了某 2 张照片，系统会立刻向 S3 并发最高优先级的 `DeleteObjectCommand` 物理级销毁远端云桶内的对应底层文件，永久杜绝图床存在垃圾冗余。
3. **Apple 全生态格式支持**：原生深度支持来自 iOS 苹果设备的 `.MOV` / `.m4v` 视频，系统会自动识别其专有的 `video/quicktime` 甚至提供首帧画面预处理嗅探，保证所有移动设备视频清晰无缝展示。
